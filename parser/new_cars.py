...
def search_new_cars():
    logger.info("Запуск поиска новых автомобилей")
    page = 1
    max_page = 2
    while page <= max_page:
        ...
        if not bearer_token:
            logger.error("Не удалось получить токен для поиска новых автомобилей")
            return
        ...
        url = "https://autospot.ru/filters/?sort=-percent_discount&limit=12&page="
    ...
    try:
        ...
        if response and response.status_code == 200:
            ...
            if match:
                ...
                if car_key:
                    logger.info("Найдены базовые данные об автомобиле")
                    car_data = data[car_key].get('body', {})
                else:
                    logger.warning("Не найден ключ с базовой информацией об автомобиле")
                ...
                if price_key and 'body' in data[price_key] and 'prices' in data[price_key]['body']:
                    logger.info("Найдена информация о ценах")
                    price = data[price_key]['body']['prices']
                else:
                    logger.warning("Не найден ключ с ценами или неверная структура")
                ...
                dealers_list = []
                if dealer_key and 'body' in data[dealer_key] and 'items' in data[dealer_key]['body']:
                    logger.info("Найдена информация о дилерах")
                    dealers = data[dealer_key]['body']['items']
                    logger.info(f"Найдено {len(dealers)} дилеров")
                    for dealer in dealers:
                        dealer_info = {
                            'name_dealer': dealer.get('dealer_group_name', None),
                            'phone_dealer': dealer.get('phone', None)
                        }
                        dealers_list.append(dealer_info)
                        logger.info(f"Добавлен дилер: {dealer_info['name_dealer']}")
                else:
                    logger.warning("Не найден ключ с информацией о дилерах или неверная структура")
            else:
